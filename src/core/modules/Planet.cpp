...
void Planet::draw3dModel(StelCore* core, StelProjector::ModelViewTranformP transfo, float screenSz, bool drawOnlyRing)
{
	...
	// V0.15: draw the solar halo before the 3D sphere.
	...
	if (screenSz>1.)
	{
		...
		if (this==ssm->getSun())
		{
			...
			// pre-0.15.2 this branch's formula was:
			//sPainter->setColor(overbright, pow(0.85f, 0.85f*extinctedMag)*overbright, pow(0.6f, 0.8f*extinctedMag)*overbright);
		}
	...
	if ((hasHalo() || this==ssm->getSun()) && allowDrawHalo)
	{
		...
		//if (englishName=="Sun")
		//	haloMag=qMin(haloMag, -18.f);
		core->getSkyDrawer()->postDrawSky3dModel(&sPainter, Vec3f(tmp[0], tmp[1], tmp[2]), surfArcMin2, haloMag, haloColorToDraw);
